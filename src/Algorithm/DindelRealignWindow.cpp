...
{
    std::tr1::unordered_map<std::string, std::pair<int, int> >::const_iterator it = this->m_variant_to_pos.find(variant.getID());
...
DindelRealignWindow::ReadSamples::ReadSamples(const std::vector<DindelRead>& dindelReads)
{
    ...
    typedef std::tr1::unordered_map<std::string, std::list<int> > Sample_to_read;
...
void DindelRealignWindow::algorithm_hmm(VCFFile & vcfFile)
{
    ...
    DindelRealignWindowResult result = estimateHaplotypeFrequencies(realignParameters.minLogLikAlignToRef, realignParameters.minLogLikAlignToAlt, true, true);
...
void DindelRealignWindow::addDiploidGenotypes(DindelRealignWindowResult& result, bool useEstimatedHaplotypeFrequencies = false)
{
   ...
   typedef std::tr1::unordered_map<std::string, std::list<int> > SampleToReads;
...
std::string DindelRealignParameters::getDefaultParameters() const
{
     ...
     paramString += ",singleSampleHetThreshold:20,singleSampleHomThreshold:20";
     return paramString; 
}	   
